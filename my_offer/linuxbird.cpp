...
计算机：接受用户输入的指令与数据，经过处理器的数据与逻辑单元运算处理后，以产生或存储成有用的信息
 0.1.1 计算机硬件的五大单元 
      『 输入单元(鼠标，键盘等)， CPU(算术逻辑，控制，记忆)， 输出单元(屏幕，打印机等) 』
       CPU 为一个具有特定功能的芯片，里头含有微指令集。
       CPU 读取的数据都是从内存中读取来的，内存的数据是由输入单元传输进来的。CPU处理完毕的数据
         要写入内存，内存再到输出单元。
 ...
 *3 文件搜索命令  [ which, find, locate, updatedb, grep ]which -a命令在哪里,whereis -b(数据库)locate显示所有
...
添加硬盘分区
   ...
   *4 写入配置文件 (/etc/fstab)
Linux 书架   
 入门类 ：《鸟哥的Linux私房菜》，《Unix Power Tools》《Linux+ Study Guide》Linux+认证考试准备 
