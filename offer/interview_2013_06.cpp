...
[716] 定义和实现一个类的成员函数为回调函数？
 回调函数就是调用者回头调用的函数，它是一个通过函数指针调用的函数。
 如果把函数的指针(地址)作为参数传递给另一个函数，当这个指针被用为
 调用它所指向的函数时，此时就可以称它为回调函数。回调函数不是由该函数的实现方直接调用的，而是
 在特定的事件或条件发生时由另外的一方调用的，用于对该事件或条件进行响应。
  定义和实现一个类的成员函数为回调函数需要做3件事 ：
 (1) 声明 (2) 定义 
...
[729] 定义宏来比较两个数a, b的大小。 #define MAX(a,b) (abs((a)-(b)) ? (a) : (b))
[730] 宏定义与内联函数的区别
...
举一反三 ：求 1+2+...+n 不能用，乘法，流程控制语句，条件判断语句。
方法一 ：构造函数与static 成员的结合
class A {
public :
    A();
    static void init();
...
[71004] 编写 strcpy() 函数。
char* strcpy(char* dst, const char* src) { // 注意src和dst所指向的内存区域不可以重叠。
    assert((dst != NULL) && (src != NULL)); // dst 必须有足够的空间容纳 src 字符串。
    char *p = dst;
    while((*dst++ = *src++) != '\0') ;
    return p;
}
[71005] 数字与字符串的相互转化。 atoi 与 itoa.
#include <stdio.h>
/* 1), 正负号 2), 前面空格 3), 溢出 4), 只要有非数字就不合法。*//* 溢出，空串，非数字，都返回 0 处理 */
...
#include <stdio.h> /* do while 是核心， 边界处理*/
const int N = 1024;
const int M = 12;
...
[71201] 编译型语言和解释型语言的区别是什么？
  解释型语言 ： 解释型语言实现中，翻译器并不产生目标机器代码。
[71202] 如何判断一段程序是由 C 编译程序，还是由 C++ 编译程序编译的。
 answ : [__STDC__] / [USING_C]
#ifdef __cplusplus
#define USING_C 0
#else
#define USING_C 1
...
[71304] 初始化列表和构造函数初始化的区别是什么？ 
         answ : 对非内置类型有区别！赋值和初始化区别！
[71305] 类的成员变量的初始化顺序是按照声明顺序吗？ 
  answer : 类的成员变量的初始化顺序只与变量在类中的声明顺序有关，静态成员变量先与实例变量，父类成员变量先于子类成员变量，父类的构造函数先于子类的构造函数。
一般 ：
 (1) 基类的静态变量或全局变量。
 (2) 派生类的静态变量或全局变量。
 (3) 基类成员变量
 (4) 派生类成员变量
[71306] 当一个类为另一个类的成员变量时，如何对其进行初始化？ (anw:成员初始化列表)
class ABC {
public :
...
[71307] C++ 能设计实现一个不能被继承的类吗？ (构造函数私有化) Good!
class A {
public :
    static A* geta() {
        return new A;
    }
[71308] 构造函数没有返回值，那么如何得知对象是否构造成功？[anw : 3条]
 (1) 通知对象构造失败的唯一方法就是在构造函数中抛出异常。
 (2) 构造函数抛出异常将导致对象的析构函数不被执行
...
********* 7-14 虚函数 ************
[71401] C++ 中的多态种类有哪几种
 answ : 参数多态，引用多态，过载多态，强制多态。
[71402] 什么函数不能声明为虚函数？ answ : static成员函数，内联函数，构造函数。
...
[71503] C语言获取文件大小的函数是什么？
 _FILE_, _FINE_, _DATE_, STDC_, _TIME_, _FUNC
[71504] 如何只使用一条语句实现 x 是否为 2 的若干次幂的判断。 n & (n-1)
...
[71506] main() 函数执行完毕后，是否有可能会再执行一段代码。
answer 1 :
#include <stdlib.h>
...
******* 数据库 ********
[80101] 一二三四范式有何区别？
  1NF ： 是指数据库表的每一列都是不可分割的基本数据项，同一列中不能有
           多个值，即实体中的属性不能有多个值或者不能有重复的属性。
  2NF ： 满足1NF，且数据库表中的每个实例或行必须可以被唯一地区分。
  3NF ： 满足2NF，且每个非主属性都不传递依赖于R的候选键，则称R是第三范式的模式。
  BCNF： 满足3NF，且每个属性都不传递依赖于R的候选键。
...
[90102] B/S与C/S有什么区别？
 在 c/s 架构中，服务器通常采用高性能的PC，工作站或者小型机，而且采用大型数据库。客户端需要安装专用的客户端软件。
 B / S 是 Brower/Server, 客户端通常只要个浏览器，服务器安装数据库。用户界面完全通过浏览器实现，一部分事务逻辑在前端实现，主要事务逻辑在服务器端实现。浏览器通过Web服务器同数据库进行数据交互。
[90103] MVC 模型结构是什么？
 模型(Model), 视图(View), 控制(Controller).它的目的是实现web系统的职能分工。
[90104] 交换机与路由器
 answ:交换机是一种基于 MAC (网卡的硬件地址)识别，能完成封装转发数据包功能的网络设备。
       它具有流量控制能力，主要用于组建局域网。
...
[90107] 什么是 ARP / RARP ？
 ARP ： 位于TCP/IP协议栈的底层协议，用于映射计算机的物理地址与网络IP地址。
     ARP 的工作就是在主机发送帧前，根据IP地址获取MAC地址，以保证通信过程的顺畅。
[90108] Ping 命令是什么？
